    ...
    extends SpecOpsAgent
{
                               ...
                               StateView state)
    {
        ...
        for (Integer enemyUnitId:enemyUnitIds){
            ...
            float decayFactor = (float) Math.exp(-timeSinceChange / 20);
            risk = decayFactor*(float)(140*Math.sin(oldProximity-newProximity))+(float)(-(0.25)*Math.pow((double)newProximity, 2)+140);
            if (risk < 0){
                risk = 0;
            }
    ...
    public boolean shouldReplacePlan(StateView state)
    {
        ...
        if (plan.size() == 0 && (DistanceMetric.chebyshevDistance(myLocation, enemyBaseLocation)) <= 1){
        ...
        for (int i = plan.size()-1;i >= Math.max(plan.size()-5,0);i--){
            for (Integer enemyUnitId:enemyUnitIds){
                Vertex enemyLocation = new Vertex(state.getUnit(enemyUnitId).getXPosition(),state.getUnit(enemyUnitId).getYPosition());
                if (DistanceMetric.chebyshevDistance(plan.get(i), enemyLocation) <= (float)2 || DistanceMetric.chebyshevDistance(enemyLocation, myLocation) <= (float)6){
                    timeSinceChange = 0;
